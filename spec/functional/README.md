...
AI agent instance management service. Runs as a long-lived container process managing named Claude Agent SDK instances — Anthropic's "Pattern 2: Long-Running Sessions". No auth (internal service, trusted network). Stateful in-memory sessions (no crash recovery).

## Architecture

```
Vercel App ──HTTP──▶ AAS Container (Hono + Node.js)
